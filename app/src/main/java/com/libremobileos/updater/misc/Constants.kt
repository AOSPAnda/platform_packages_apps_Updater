...
object Constants {
    ...
    const val PROP_BUILD_VERSION = "ro.lmodroid.version"
    const val PROP_BUILD_VERSION_INCREMENTAL = "ro.build.version.incremental"
    const val PROP_DEVICE = "ro.lmodroid.device"
    const val PROP_NEXT_DEVICE = "ro.updater.next_device"
    const val PROP_RELEASE_TYPE = "ro.lmodroid.build_type"
    const val PROP_UPDATER_ALLOW_DOWNGRADING = "lmodroid.updater.allow_downgrading"
    const val PROP_UPDATER_URI = "lmodroid.updater.uri"
    const val PROP_UPDATER_ALLOW_MAJOR_UPDATE = "lmodroid.updater.allow_major_update"
